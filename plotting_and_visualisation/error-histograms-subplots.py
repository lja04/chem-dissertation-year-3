...
def main():
    input_dir = "csv-files/split-files"
    ...
    # Create subplots histograms
    fig, axes = plt.subplots(len(k_point_errors), 1, figsize=(12, 4*len(k_point_errors)), sharex=True)
    fig.suptitle('Histograms of Errors at Each K-point Spacing')

    for (k_point, errors), ax in zip(sorted(k_point_errors.items()), axes):
        ax.hist(errors, bins=20)
        ax.set_ylabel(f'K-point {k_point}')
        ax.set_xlabel('Error')

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'k_point_error_histograms_subplots.png'))
    plt.close()
